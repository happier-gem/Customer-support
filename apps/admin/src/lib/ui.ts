...
export function planBadgeClass(plan: string): string {
  return `inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${PLAN_BADGE_CLASSES[plan] ?? "bg-muted text-muted-foreground"}`;
}
...
export function statusBadgeClass(isSuspended: boolean): string {
  return `inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${
    isSuspended ? "bg-danger/10 text-danger" : "bg-success/10 text-success"
  }`;
}
