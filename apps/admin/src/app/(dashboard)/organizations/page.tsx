  ...
  return (
        ...
        <div className={cardClass}>
          {loading ? (
            ...
            <p className="text-sm text-muted-foreground">No organizations match these filters.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[720px] text-left text-sm">
                ...
                <tbody className="divide-y divide-border">
                  {result.data.map((org) => (
                    <tr key={org.id} className="hover:bg-muted">
                      <td className="max-w-[220px] truncate py-2 font-medium text-foreground">
                      ...
                      <td className="py-2 text-muted-foreground">{org.activeMemberCount}</td>
                      <td className="py-2 text-muted-foreground">{org.ticketCount}</td>
                      <td className="py-2 text-muted-foreground">{org.feedbackFormCount}</td>
                      <td className="py-2">
                        <span className={statusBadgeClass(org.isSuspended)}>{org.isSuspended ? "Suspended" : "Active"}</span>
                      </td>
                      <td className="py-2 whitespace-nowrap text-muted-foreground">{new Date(org.createdAt).toLocaleDateString()}</td>
                      <td className="py-2">
