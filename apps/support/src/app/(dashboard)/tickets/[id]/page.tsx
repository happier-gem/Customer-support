...
export default function TicketDetailPage() {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    await runAction(() => api.uploadAttachment(accessToken, params.id, file), "Attachment uploaded.");
  ...
  const canPickUp = user.role === "SUPPORT_AGENT" && !ticket.assignedAgent;
  const canAssign = user.role === "TENANT_OWNER";
  const canChangeStatus = user.role === "SUPPORT_AGENT" || user.role === "TENANT_OWNER";
  ...
  return (
        ...
        {(canPickUp || canAssign || canChangeStatus) && (
          <div className={`${cardClass} space-y-3`}>
              ...
              {canAssign && (
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    onClick={() => runAction(() => api.assign(accessToken!, ticket.id, selectedAgentId), "Ticket assigned.")}
