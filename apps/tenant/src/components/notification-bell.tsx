...
/** Only routes that exist in this app — a related resource this app has no page for is left un-clickable rather than 404ing. */
function notificationHref(n: Notification): string | null {
  if (n.ticketId) return `/tickets/${n.ticketId}`;
  if (n.invitationId) return "/settings/team";
...
export function NotificationBell() {
  ...
  // dedicated notification socket (see plan.md's real-time scope decision).
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute right-0.5 top-0.5 flex h-4 min-w-[1rem] -translate-y-1/2 translate-x-1/2 items-center justify-center rounded-full bg-danger px-1 text-[10px] font-semibold leading-none text-white">
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-2 w-80 max-w-[90vw] overflow-hidden rounded-lg border border-border bg-card shadow-lg">
          <div className="flex items-center justify-between border-b border-border px-4 py-2.5">
            <span className="text-sm font-semibold text-foreground">
              Notifications{unreadCount > 0 ? ` · ${unreadCount} unread` : ""}
            </span>
            {unreadCount > 0 && (
              <button onClick={handleMarkAllRead} className="text-xs font-medium text-muted-foreground hover:text-foreground">
                Mark all read
              </button>
            )}
          </div>
          <div className="max-h-96 overflow-y-auto">
            {loading && notifications.length === 0 ? (
              ...
              <p className="px-4 py-6 text-center text-sm text-muted-foreground">Unable to load notifications.</p>
              ...
              notifications.map((n) => (
                <button
                  ...
                  className={`block w-full border-b border-border/60 px-4 py-3 text-left last:border-b-0 hover:bg-muted ${n.read ? "" : "bg-info/10"}`}
                >
